...
function CheckoutCardSection(){
  ...
  return(
    <div className="px-2 py-6 h-screen md:grid md:auto-rows-min">
      <h1 className="mb-8 text-center md:text-lg">Add your card</h1>
      <form aria-label="form" onSubmit={(e)=>{
        e.preventDefault()
        handleSubmit.mutate()
      }}  className="grid md:w-fit md:place-self-center">
        ...
        <div className={isAddressAndBillingSame ? 'hidden':'grid'}>
          ...
          <button className="bg-red-700 text-gray-200 py-2 px-6 rounded-md font-semibold w-full mb-8 md:w-fit md:px-16 place-self-center text-center" onClick={()=>setCanShowFinalButton(true)} name="submit-address">Use this address</button>
        </div>
        ...
        <button className={`bg-red-700 text-gray-200 py-2 px-6 my-4 rounded-md font-semibold w-full ${canShowFinalButton ? '':'hidden'} md:w-fit md:px-16 place-self-center text-center`} onSubmit={e=>e.preventDefault()} type="submit" name="submit">Submit my order</button>
      </form>
    </div>
  )
}
